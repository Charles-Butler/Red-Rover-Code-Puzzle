...
parser.add_argument('-d', '--desc', action='store_true', help='Sorts in Descending (Z-A) order Defaults to Asc (A-Z)')
...
def split_then_sort(usr_str, desc=False):
    items = []
    current_item = ""
    n_level = 0
...
second_str=split_then_sort(STRING, desc)
...
if desc:
    print(f"\nSorted Output in Descending Order:")
else:
